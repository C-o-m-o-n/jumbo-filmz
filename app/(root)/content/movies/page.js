...
export default function Movies() {
  ...
  function trendingTrailerBtn() {
    return (
      <div className='flex flex-row items-center mt-3  transition-all duration-300 ease-in-out bg-[#b70000] rounded-3xl'>
        <FaEye size={25} className='ml-2' />
        <span className="select-none flex items-center px-3 py-2 cursor-pointer rounded-[.95rem]">
          <p className="flex items-center flex-grow hover:text-white">Watch Trailer</p>
        </span>
      </div>
    )
  }
    ...
    <main className="flex min-h-screen flex-col items-start md:ml-[210px] md:mt-[80px]">
      ...
      <div className={`${showVideoPlayer && 'blur-bg'} flex flex-row gap-5`}>
              ...
              <div className="absolute md:rounded-2xl bg-[black] opacity-50 h-full top-0  w-full">
                <div>
                  <div className="absolute md:ml-12 top-[100px] md:top-[200px] flex flex-col justify-center items-ceter">
                    ...
                    <p className="hidden md:block text-center md:text-left text-[#efefef]">{randomMovie.overview}.</p>
                    ...
                    <div className="flex flex-row items-center md:mt-3 gap-3">
                      ...
                      <div className='flex flex-row items-center w-[132px] transition-all duration-300 ease-in-out bg-[#383e52] rounded-3xl'>
                        ...
                        <span className="select-none flex items-center px-4 py-2 cursor-pointer my-[.4rem] rounded-[.95rem]">
                          <p className="flex items-center flex-grow hover:text-white">Save</p>
                        </span>
                      </div>
                    </div>
                  </div>
        ...
        <div className="hidden md:block">
          ...
          <MovieList showName={true} showOverlayBtns={false} urlType={trendingUrl} trendingTrailerBtn={trendingTrailerBtn}
            divStyle={"flex flex-row w-[270px] gap-3 items-center rounded-2xl bg-[#23252e] mt-3"}
            imageStyle={"rounded w-[100px] h-[100px]"} customStyle={'hidden md:flex flex-col gap-3 justify-center items-center w-auto md:w-full'} limit={3} />
      ...
      <div className="w-auto mt-5 ">
        ...
        <div className={`${showVideoPlayer && 'blur-bg'}`} >
          <MovieList showName={false} discoverMovieIdSetter={setRandomMovie}
            showOverlayBtns={true} urlType={discoverUrl} imageStyle={" rounded w-full h-full"}
            customStyle={'hidden md:flex flex-row gap-3 justify-center items-ceter md:w-full'} limit={6} />
        ...
        <div class="grid grid-cols-2 md:hidden gap-4">
          {ApiResponse && ApiResponse.slice(0, 4).map((movie) => (
            <div key={movie.id} className="card grid gap-4">
              <div className="button-grp ">
                <FaEye
                  onClick={() => setRandomMovie(movie)} size={35} color='white'
